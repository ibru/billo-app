...
- **App Store review prompts**: `ReviewPromptModel` (`Billo/Utilities/`, env-injected from `BilloApp`) is the single decision point for `requestReview` triggers: in-app paywall purchase, caught-up mark-paid (≥3 recorded payments + `BillsModel.isCaughtUp`), 10th-bill / 2nd-income milestones (once each), and the onboarding purchase thank-you. **No manual time cooldown by design** — iOS caps the dialog at 3 displays per 365 days and decides whether to show; our gates only pick quality moments. Present via `requestReview.requestAfterSettleDelay()` (cancellation-safe 1 s delay; exception: the onboarding thank-you screen calls bare `requestReview()` after its own cancellation-guarded 1 s `.task` — don't "clean that up" into the helper). Disabled in SCREENSHOTS builds. Analytics: `rating prompt requested` with a `trigger` property (`ReviewPromptTrigger` raw values).
