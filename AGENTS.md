...
- Screenshots run (in-memory DB pre-seeded with realistic demo data): use the `BilloScreenshots` scheme. It launches the main app with the `Screenshots` build configuration, which sets the `SCREENSHOTS` compile condition — see `Billo/App/ScreenshotMockData.swift` and the `#if SCREENSHOTS` branches in `BilloApp.swift`. The seed data is market-aware (US default; `it`, `es`, `mx`, `br` datasets with local brands, currency, and pay cadence) — select via the `SCREENSHOT_MARKET` env var on the scheme, or by setting the run's App Region (the fallback signal). Capture-automation launch arguments (SCREENSHOTS builds only): `-billsDefaultView list|calendar` picks the home view, `-screenshotRealNotifications` swaps the Noop notification center for the real one and requests authorization (enables `simctl push` lock-screen frames), `-screenshotLandscape` locks landscape for headless iPad captures (works because the Screenshots configuration sets `INFOPLIST_KEY_UIRequiresFullScreen`).
- App Store screenshots (all locales, fully scripted per device): `scripts/capture-appstore-iphone-shots.sh --all` (iPhone 17 Pro sim, portrait, incl. lock-screen notifications), `scripts/capture-appstore-ipad-shots.sh --all` (iPad (A16) sim, landscape), `scripts/capture-appstore-mac-shots.sh --all` (Mac Catalyst on the host). Shot catalogue, required screen states, and every platform trap live in `docs/appstore-shots.md` — read it before changing a script or re-shooting. The Mac app's default window size lives in `MacWindowDefaults.defaultSize` (`Billo/Utilities/`); keep it in sync with the mac script's pinned size.
...
- **macOS 26 (Tahoe) Catalyst platform bugs** (verified on macOS 26.3.1, empirically diagnosed): (1) SwiftUI sheet dismissal never reaches UIKit — the binding nils and the parent re-renders, but the presented controller stays on screen (buttons dead) until the app deactivates. Workaround: dismiss by clearing the presentation binding, then complete the teardown with a Catalyst-only UIKit sweep (`OnboardingBillSetupStepView.dismissAdjustSheet`); other sheets (`MarkPaidSheet`, `DayDetailSheet`, `BillEditView`, …) likely need the same treatment before a macOS release. (2) SwiftData save-crash, ROOT CAUSE FOUND (2026-08-05, production iOS crash reports): ANY `context.save()` can crash (EXC_BREAKPOINT, "Unexpectedly found nil" in `ModelContext.container.getter` under `_SwiftData_SwiftUI.QueryController.didSaveChange`) **after the onboarding showcase container has been deallocated** — the showcase (`OnboardingViewModesStepView` → `HomeShowcaseEnvironment`) embeds the real `BillsListView`/`BillsCalendarView`, whose `@Query` controllers outlive the throwaway in-memory container and blow up on the NEXT didSave notification from the main context (observed via `deleteIncome` on iOS 26.5.2, and earlier via `PaymentRecorder` on Catalyst — same crash, misattributed to OS skew; Catalyst tests pass only because tests never run onboarding). Fix direction: keep the showcase container alive for the process lifetime (static cache) or stop embedding `@Query`-backed views in throwaway containers. (3) `@Observable` environment values do not propagate into presented sheet/popover content — the first `@Environment(SomeModel.self)` read inside the presented tree traps with "No Observable object of type … found" (reproduced: MarkPaidSheet's partial-payment gate presenting the paywall). Rule: EVERY `.sheet`/`.popover` content root must re-inject the app models via `.appEnvironment(_:)` (`Shared/AppEnvironmentModels.swift`), plus any screen-scoped models (`BillModel`, `NotificationSettingsModel`) individually; views pushed inside a presented `NavigationStack` need nothing extra.
