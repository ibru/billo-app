...
- macOS App Store screenshots (Mac Catalyst, all locales): `scripts/capture-appstore-mac-shots.sh --all` — shot catalogue and Catalyst capture traps in `docs/appstore-shots.md`. The Mac app's default window size lives in `MacWindowDefaults.defaultSize` (`Billo/Utilities/`); keep it in sync with the script's pinned size.
